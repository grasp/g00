...
require File.join(project_root,"test","watir","w090_test_common.rb")
  ...
  #  big_cate,huo_dalei,huo_xiaolei,pack_dalei,pack_xiaolei
    
     browser.select_list(:id, "stockcargo_big_category").set("大件货物")
     browser.link(:text, "类别选择").click; ;sleep 1
     browser.link(:href, "#{site_root}/cargo_categories/show/010000").click;sleep 0.5
     browser.link(:href, "#{site_root}/cargo_categories/show/010200").click;sleep 0.5
     browser.link(:id, "list_closec").click;
     
    browser.link(:text, "包装选择").click;sleep 0.5
    
    browser.link(:href, "#{site_root}/package_categories/show/9000").click;sleep 0.5
    browser.link(:href, "#{site_root}/package_categories/show/9993").click;sleep 0.5

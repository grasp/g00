    ...
    if session[:user_id]
      ...
      unless @concerncargo.blank?        #to get all city cargo 
        ...
        unless @concerncargo.city.blank? 
          ...
          @cityconcerncargo=Cargo.where(:status=>"正在配车").any_in(:fcity_code=>city_array).desc(:created_at).limit(10)
        end      
        ...
        unless @concerncargo.line.blank? 
          ...
          @concerncargo.line.each {|x| line_array<<x[0]}
          @lineconcerncargo=Cargo.where(:status=>"正在配车").any_in(:line=>line_array).desc(:created_at).limit(10)
        end       
     ...
     unless @concerncargo.userid.blank? 
         user_array=Array.new
          ...
          @userconcerncargo=Cargo.where(:status=>"正在配车").any_in(:user_id=>user_array).desc(:created_at).limit(10)
        end       
         ...
         phone_array=Array.new
          ...
          @phoneconcerncargo=Cargo.where(:status=>"正在配车").any_in(:phone>phone_array).desc(:created_at).limit(10)
        end       
